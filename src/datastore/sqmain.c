...
void sqlite3RegisterGlobalFunctions() {
  // We don't do sql so we don't have any functions to register
}

void sqlite3RegisterBuiltinFunctions(sqlite3* pDb) {
  // And again
}

void sqlite3_reset_auto_extension() {
  // This is in loadext.c which has many dependencies. This is only shutdown code
  // so probably safe to omit.
}

void sqlite3AutoLoadExtensions(sqlite3* pDb) {
}

void sqlite3CloseExtensions(sqlite3* pDb) {
}

void sqlite3ExpirePreparedStatements(sqlite3* pDb) {
}

int sqlite3VtabRollback(sqlite3* pDb) {
  return SQLITE_OK;
}
...
// We can never create a Parse* so we should never need this...
...
int sqlite3_open_v2(const char *zFilename, sqlite3 **ppDb, int flags, const char *zVfs) {
    return openDatabase(zFilename, ppDb, flags, zVfs);
}
