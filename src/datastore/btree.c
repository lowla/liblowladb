...
** SQLite database.
*/
static const char zMagicHeader[] = "Teamstudio Db 1"; // SQLITE_FILE_HEADER;
...
/*
 ** The first argument, pCur, is a cursor opened on some b-tree. Count the
 ** number of entries in the b-tree up to and including the current position
 ** and write the result to *pnEntry.
 **
 ** SQLITE_OK is returned if the operation is successfully executed. 
 ** Otherwise, if an error is encountered (i.e. an IO error or database
 ** corruption) an SQLite error code is returned.
 */
int sqlite3BtreePos(BtCursor *pCur, i64 *pnEntry){
    i64 nEntry = 0;                      /* Value to return in *pnEntry */
    int rc;                              /* Return code */
    ...
    for (int i = 0 ; i <= iPage ; ++i) {
        aiIdx[i] = pCur->aiIdx[i];
    }
    int lastIdx = -1;
    if (pCur->apPage[pCur->iPage]->leaf) {
        lastIdx = aiIdx[iPage];
        aiIdx[iPage] = 0;
    }
    ...
    while (rc == SQLITE_OK) {
        ...
        if (pPage->leaf) {
            nEntry += pPage->nCell;
            pCur->aiIdx[pCur->iPage] = pPage->nCell - 1; // -1 since it gets incremented in BtreeNext
        }
        else {
            ++nEntry;
        }
